...
- **Syntax analysis:** The create_cmd() function (implemented in parser.c) parses the list of tokens and constructs a command structure, including redirections and heredocs.
  - If a << heredoc is encountered, create_heredoc() (implemented in `heredoc.c`) reads the content from the user until the delimiter and stores it in the command structure.
...
### Exemple of allocation workflow:
```c
